...
const Franchise = ({ closeModal }) => {
  ...
  const handleDelete = async (fid) => {
    ...
    try {
        await axios.delete(`https://winkget-8.onrender.com/api/Franchisedelete/${fid}`);
  ...
  const fetchVender = async () => {
    try {
      const response = await axios.get('https://winkget-8.onrender.com/api/Franchiseget');
...
setImage('')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://winkget-8.onrender.com/api/Registerf', formData);
