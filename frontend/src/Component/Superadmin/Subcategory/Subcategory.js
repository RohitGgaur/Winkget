...
const Subcategory = () => {
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      try {
        const response = await axios.get(`https://winkget-8.onrender.com/api/categories/${id}`);
  ...
  const handleFileChange = async (e) => {
    ...
    formDataToSend.append('upload_preset', 'eq2in9eu'); // Replace with Cloudinary upload preset
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `https://winkget-8.onrender.com/api/categories/${selectedSubcategory._id}`,
        formDataToSend
      );
  ...
  const handleDelete = async (subcategoryId) => {
    ...
    if (window.confirm("Are you sure you want to delete this subcategory?")) {
      try {
        await axios.delete(`https://winkget-8.onrender.com/api/categories/${subcategoryId}`);
