...
const Update = ({ showModal, onClose, id }) => {
    ...
    useEffect(() => {
        if (id && showModal) {
            const fetchData = async () => {
                setLoading(true);
                try {
                    const response = await axios.get(`https://winkget-8.onrender.com/api/Adminget/${id}`);
                    if (response.data) {
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.put(`https://winkget-8.onrender.com/api/admin-update/${id}`, data);
